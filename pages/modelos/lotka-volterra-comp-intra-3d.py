...
from utils import modelo_lv_comp_intra_3d

dash.register_page(
    __name__,
    path='/modelos/lotka-volterra-comp-3d',
...
layout = html.Div(className='pages', children = [
    html.H2('Modelo Depredador-Presa (Lotka-Volterra) con Competencia Intraespecifica', className='page_title'),
        ...
        html.Div(className='div_grafica', children = [
            ...
            html.Div(className='grafica', children = [
                # dcc.Loading(
                    # type='dot',
                    # children=
                    dcc.Graph(id='figure_lv_cmp_3d',style={'width': '100%', 'height': '700px'})
                # ),
            ])                        
...
@callback(
    Output('figure_lv_cmp_3d', 'figure'),
    Input('X0', 'value'),
    Input('Y0', 'value'),
    Input('Z0', 'value'),
    Input('a21', 'value'),
...
def grafica_lv(X0,Y0,Z0,a21,a22,a23,a31,a32,a33,d1,d2,t,cant):
    fig = modelo_lv_comp_intra_3d(X0,Y0,Z0,a21,a22,a23,a31,a32,a33,d1,d2,t,cant)
    return fig
